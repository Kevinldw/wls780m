...
public class ByteUtils {
    ...
    public static String byteToString(byte[] bytes, int start, int length) {
        StringBuilder sb = new StringBuilder();
    ...
    public static int byteToInt(byte[] bytes, int lstart){
        return bytes[lstart + 0] & 0xFF | (bytes[lstart + 1] & 0xFF) << 8 | (bytes[lstart + 2] & 0xFF) << 16 | (bytes[lstart + 3] & 0xFF) << 24;
    }
    ...
    public static boolean byteEquals(byte[] lbytes, byte[] rbytes, int length) {
        if (lbytes == null && rbytes == null) {
            return true;
        }
    ...
    public static void copy(byte[] lbytes, byte[] rbytes, int lstart, int rstart, int length) {
        if (lbytes != null && rbytes != null && lstart >= 0) {
            for (int i = lstart, j = rstart, k = 0; k < length; i++, j++, k++) {
                lbytes[i] = rbytes[j];
            }
        }
    }
    ...
    public static int byteToShort(byte[] bytes, int lstart){
        return bytes[lstart + 0] & 0xFF | (bytes[lstart + 1] & 0xFF) << 8;
    }
}
